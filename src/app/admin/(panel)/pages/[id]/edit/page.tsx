...
export default async function AdminEditPagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className={adminPage}>
      ...
      <AdminForm
        title={isAboutPage ? 'Edit About Us page' : 'Edit page'}
        subtitle={isAboutPage ? 'Structured sections map directly to the public /about layout.' : page.title}
        action={update}
        className={isAboutPage ? 'max-w-3xl' : 'max-w-2xl'}
      >
        {isAboutPage ? (
